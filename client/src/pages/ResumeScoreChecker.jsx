...
const ResumeScoreChecker = () => {
    ...
    const runDeepAnalysis = async () => {
        if (resumeText.trim().length < 40 || aiLoading) return;
        setAiLoading(true);
        setAiError('');
        try {
            const { data } = await api.post('/ai/deep-analyze-resume', { resumeText, jobDescription });
            setAiAnalysis(data);
        } catch (error) {
            setAiAnalysis(null);
            setAiError(error.response?.data?.message || 'Deep analysis is temporarily unavailable. Your local score is still available.');
        } finally {
            setAiLoading(false);
        }
    };
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div className="mt-12 grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
                    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl shadow-slate-200/50 sm:p-7 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none" aria-labelledby="paste-resume-heading">
                        ...
                        <button type="button" onClick={() => { setHasAnalyzed(true); setAiAnalysis(null); setAiError(''); }} disabled={resumeText.trim().length < 40} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-orange-500 px-5 py-4 text-sm font-black text-white shadow-lg shadow-orange-500/20 transition hover:bg-orange-600 disabled:cursor-not-allowed disabled:bg-slate-300 disabled:shadow-none dark:disabled:bg-slate-700"><ClipboardCheck className="h-5 w-5" /> Check my resume</button>
                        <button type="button" onClick={runDeepAnalysis} disabled={resumeText.trim().length < 40 || aiLoading} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-orange-200 bg-orange-50 px-5 py-3.5 text-sm font-black text-orange-700 transition hover:border-orange-400 hover:bg-orange-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-orange-500/30 dark:bg-orange-500/10 dark:text-orange-200 dark:hover:bg-orange-500/20">{aiLoading ? <LoaderCircle className="h-5 w-5 animate-spin" /> : <Lightbulb className="h-5 w-5" />} {aiLoading ? 'Gemini is reviewing your draft…' : 'Get Gemini deep analysis'}</button>
                        <p className="mt-3 text-center text-[11px] leading-5 text-slate-400">Optional AI coaching uses the configured server-side Gemini key. Your local score never depends on AI.</p>
